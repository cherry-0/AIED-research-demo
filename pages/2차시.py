...
st.text_area("1차시에서 그린 사각형 이미지를 기존 데이터셋에 추가한 후 결과를 확인해봅시다 새로 추가한 사각형 이미지가 잘 분류되었다고 생각하나요 그 이유는 무엇인가요?")

st.success("인공지능 멘티가 더 정확하게 사각형 이미지를 분류하려면 어떤 모양의 사각형 이미지를 데이터셋에 추가하면 좋을지 생각해 보고 차시의 Algeomath를 이용해 원하는 만큼 더 그려 봅시다. 개수에 제한은 없습니다 ")
"###### 훈련 데이터셋 수정하기"
...
"### 아이디어 공유하기 "
#st.success("이번 시간을 통해 알아낸 좋은 전략에 대해 자랑해주세요")

st.text_area("이번 시간을 통해 알아낸 좋은 전략에 대해 자랑해주세요")
